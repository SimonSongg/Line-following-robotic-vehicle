...
const unsigned int SG90_DOWN_VALUE =950;  //These two should be less than 20000
...
void *thread_ReadCamera(void *args)
{
#ifdef FPS
    static TickMeter tm;
    static int count = 1;
...
void *thread_HandleEvent(void *args)
{
#ifdef FPS_2
    static TickMeter tm;
    static int count = 1;
...
inline Mat ReadCamera()
{
    Mat temp;
    flip(originalMat, temp, -1);
    return temp;
}
...
int main()
{
    ...
    cout << " (FPS ON)";
#endif
    cout << endl << "Press enter to stop when running." << endl;

    cin.get();

    pthread_t tid_camera;
    int tret_camera = pthread_create(&tid_camera, NULL, thread_ReadCamera, NULL);
    if (tret_camera != 0)
        return -1;

    pthread_t tid_event;
    int tret_event = pthread_create(&tid_event, NULL, thread_HandleEvent, NULL);
    if (tret_event != 0)
        return -1;

    pthread_t tid_exit;
    int tret_exit = pthread_create(&tid_exit, NULL, thread_Exit, NULL);
    if (tret_exit != 0)
        return -1;

    pthread_join(tid_exit, NULL);
    pthread_cancel(tid_event);
    pthread_cancel(tid_camera);
...
bool CheckEvent()
{
    ...
    Mat inMat, cutMat, cutMatHSV, cutMatFilted;
    inMat = ReadCamera();
    cutMat = inMat.rowRange((int)(EVENT_CHECK_MAT_Y * inMat.rows),
                            (int)((EVENT_CHECK_MAT_Y + EVENT_CHECK_MAT_Y_RANGE) * inMat.rows));
...
State CheckOperation()
{
    ...
    Mat symbolMat = imgTransformed(symbolTargetRect); //Maybe deleted????
...
void FollowLinePID(Scalar lower, Scalar upper, double basespeed, bool pidZero)
{
    ...
    Mat inputMat, cutMat, cutMatHSV, cutMatFilted;
    inputMat = ReadCamera();
    cutMat = inputMat.rowRange((int)(LINE_FOLLOWING_MAT_Y * inputMat.rows),
                               (int)((LINE_FOLLOWING_MAT_Y + LINE_FOLLOWING_MAT_Y_RANGE) * inputMat.rows));
    cvtColor(cutMat, cutMatHSV, COLOR_BGR2HSV, 0);
    inRange(cutMatHSV, lower, upper, cutMatFilted);

#ifdef DEBUG_IMSHOW
    imshow("originalMat", inputMat);
...
//    }
    ...
    cout << "PID: LOWER-" << lower[0] << ',' << lower[1] << ',' << lower[2]
         ...
         << " pidZero-" << pidZero << " rows-" << inputMat.rows << endl;
...
void HandleShortCut(int colorIndex)
{
    ...
    while (1)
    {
        ...
        if (countNonZero(cutMatBinary) > 150)
        {
            isStart = true;
