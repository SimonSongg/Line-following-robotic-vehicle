/*
CODING WORK OF GROUP 6 IN AEEE MODULE
THIS PROGRAM IS USED FOR MULTI-TASK TEST
INCLUDING COUNT-SHAPE TEST AND KICK-FOOTBALL TEST
*/
...
unsigned int SG90_DOWN_VALUE = 950;         //These two should be less than 20000 //950
const unsigned int SG90_MOVE_PERIOD = 1000; //miliseconds

//Line Following Parameters
const int LINE_FOLLOWING_BLANK = 10;
double LINE_FOLLOWING_MAT_Y = 0.5;             //<-
...
const double SHORTCUT_BLUE_BASESPEED = 25;
const double SHORTCUT_GREEN_BASESPEED = 30;
const double SHORTCUT_RED_BASESPEED = 20;
const double SHORTCUT_YELLOW_BASESPEED = 25;
...
//TurnLeft Parameters
const int TURN_LEFT_MOTOR_SPEED = 70;
const int TURN_LEFT_PERIOD = 1700;

//TurnRight Parameters
const int TURN_RIGHT_MOTOR_SPEED = 70;
const int TURN_RIGHT_PERIOD = 1700;

//KickFootball
const int KICK_FOOTBALL_TURN_LMOTOR_SPEED = -50;
const int KICK_FOOTBALL_TURN_RMOTOR_SPEED = 50;
const unsigned int KICK_FOOTBALL_TURN_PERIOD = 1;
const int KICK_FOOTBALL_FORWARD_SPEED = 18;
const unsigned int KICK_FOOTBALL_FORWARD_PERIOD = 1;
const int KICK_FOOTBALL_BACKWARD_SPEED = -18;
const unsigned int KICK_FOOTBALL_BACKWARD_PERIOD = 1;
const int KICK_FOOTBALL_TURNB_LMOTOR_SPEED = 50;
const int KICK_FOOTBALL_TURNB_RMOTOR_SPEED = -50;
const unsigned int KICK_FOOTBALL_TURNB_PERIOD = 2;
...
const double COUNT_SHAPE_THRESHOLD_RANGE_UPPER = 255;
const double COUNT_SHAPE_THRESHOLD_RANGE_LOWER = 150;
const int COUNT_SHAPE_MEDIANBLUR_SIZE = 3;
const double COUNT_SHAPE_AREA_RANGE_UPPER = 90000.0;
const double COUNT_SHAPE_AREA_RANGE_LOWER = 100.0;
const double COUNT_SHAPE_APPROXPOLY_EPSILON = 14.0;
...
void HandleEvent()
{
    ...
    if (CheckEvent())
    {
        LINE_FOLLOWING_MAT_Y = 0.5; //0.5
        SG90_DOWN_VALUE = 950;      //950
        ...
        State tempState1 = CheckOperation();
        State tempState2 = CheckOperation();
        ...
        {
            State tempState3 = CheckOperation();
            if (tempState3 == tempState1)
                stateNew = tempState1;
            else if (tempState3 == tempState2)
                stateNew = tempState2;
            else
...
State CheckOperation()
{
    ...
    Mat symbol = MatchSymbol(originalMat);
    if (symbol.empty())
    {
    ...
    cvtColor(symbol, symbolHSV, COLOR_BGR2HSV, 0);
...
Mat MatchSymbol(Mat img)
{
    ...
    cvtColor(img, imgHSV, COLOR_BGR2HSV, 0);
    ...
    Mat imgTransformed;
    warpPerspective(img, imgTransformed, warpMatrix, imgTransformed.size(), INTER_LINEAR);

    Mat symbolMat = imgTransformed(symbolTargetRect);
...
//pidZero: true -> set three pid parameters and blankCount to 0
void FollowLinePID(Scalar lower, Scalar upper, double basespeed, bool pidZero)
{
    ...
    if (blankCount >= LINE_FOLLOWING_BLANK)
    {
        speedL = -LINE_FOLLOWING_BASESPEED; //<-
        speedR = -LINE_FOLLOWING_BASESPEED; //<- Let car move backwards if no black line detected
    }
...
inline int constrain(int raw, int lower, int upper)
{
    if (raw > upper)
        return upper;
    if (raw < lower)
...
void HandleShortCut(int colorIndex)
{
    ...
    while (1)
    {
        ...
        {
            isStart = true;
            FollowLinePID(lower, upper, basespeed, true);
        }
        else
        {
            if (isStart)
...
void HandleTurnLeft()
{
    ...
    DriveMotor(LINE_FOLLOWING_BASESPEED, LINE_FOLLOWING_BASESPEED);
    delay(TURN_LEFT_PERIOD);
    StopMotor();
    ClearCameraBuffer();
    DriveMotor(-TURN_LEFT_MOTOR_SPEED, TURN_LEFT_MOTOR_SPEED);
    while (1)
    {
        ReadCamera();
        Mat cutMat, cutMatHSV, cutMatFilted;
        cutMat = originalMat.rowRange((int)(LINE_FOLLOWING_MAT_Y * originalMat.rows),
                                      (int)((LINE_FOLLOWING_MAT_Y + LINE_FOLLOWING_MAT_Y_RANGE) * originalMat.rows));
        cvtColor(cutMat, cutMatHSV, COLOR_BGR2HSV, 0);
        inRange(cutMatHSV, LINE_FOLLOWING_BKCOLOR_LOWER, LINE_FOLLOWING_BKCOLOR_UPPER, cutMatFilted);
        if (countNonZero(cutMatFilted) > 20)
            return;
    }
}
...
void HandleTurnRight()
{
    ...
    StopMotor();
    ClearCameraBuffer();
    DriveMotor(TURN_RIGHT_MOTOR_SPEED, -TURN_RIGHT_MOTOR_SPEED);
    while (1)
    {
        ReadCamera();
        Mat cutMat, cutMatHSV, cutMatFilted;
...
void HandleKickFootball()
{
    ...
    sleep(KICK_FOOTBALL_TURN_PERIOD);
    DriveMotor(KICK_FOOTBALL_FORWARD_SPEED, KICK_FOOTBALL_FORWARD_SPEED);
    sleep(KICK_FOOTBALL_FORWARD_PERIOD);
    DriveMotor(KICK_FOOTBALL_BACKWARD_SPEED, KICK_FOOTBALL_BACKWARD_SPEED);
    sleep(KICK_FOOTBALL_BACKWARD_PERIOD);
    DriveMotor(KICK_FOOTBALL_TURNB_LMOTOR_SPEED, KICK_FOOTBALL_TURNB_RMOTOR_SPEED);
    sleep(KICK_FOOTBALL_TURNB_PERIOD);
    return;
}
...
void HandleCountShape()
{
    ...
    Mat symbolGray, symbolBinary;
    cvtColor(symbol, symbolGray, COLOR_BGR2GRAY);
    threshold(symbolGray, symbolBinary, COUNT_SHAPE_THRESHOLD_RANGE_LOWER, COUNT_SHAPE_THRESHOLD_RANGE_UPPER, THRESH_BINARY);
    symbolBinary = ~symbolBinary;
    medianBlur(symbolBinary, symbolBinary, COUNT_SHAPE_MEDIANBLUR_SIZE);
    ...
    vector<Vec4i> hireachy;
    findContours(symbolBinary, shapeContours, hireachy, RETR_TREE, CHAIN_APPROX_SIMPLE, Point());
    ...
    for (size_t i = 0; i < shapeContours.size(); i++)
    {
        ...
        {
            cout << "hireachy[" << i << "][" << j << "]-" << hireachy[i][j] << " / ";
        }
    }
#endif

    vector<vector<Point>> shapeContours_poly(shapeContours.size());
#ifdef DEBUG_IMSHOW
    Mat shapeContourMat(symbol.size(), CV_8UC3, Scalar::all(0));
#endif
#ifdef DEBUG_CONSOLE
    cout << "CountShape: ";
#endif
    for (size_t i = 0; i < shapeContours.size(); i++)
    {
        double area = contourArea(shapeContours[i]);
#ifdef DEBUG_CONSOLE
        cout << "Area[" << i << "]-" << area << " / ";
#endif
        if (area < COUNT_SHAPE_AREA_RANGE_LOWER || area > COUNT_SHAPE_AREA_RANGE_UPPER)
        {
            continue;
        }
        approxPolyDP(shapeContours[i], shapeContours_poly[i], COUNT_SHAPE_APPROXPOLY_EPSILON, true);

        if (shapeContours_poly[i].size() == 3)
            triangleCount++;
        else if (shapeContours_poly[i].size() == 4)
            squareCount++;
        else if (shapeContours_poly[i].size() > 4)
            circleCount++;

#ifdef DEBUG_IMSHOW
        drawContours(shapeContourMat, shapeContours_poly, i, Scalar(0, 255, 255), 2, 8);
#endif
    }

#ifdef DEBUG_CONSOLE
    cout << "CountShape: "
         << "Tri-" << triangleCount << " Sq-" << squareCount << " Cir-" << circleCount << endl;
#endif
#ifdef DEBUG_IMSHOW
    imshow("shapeContourMat", shapeContourMat);
    waitKey(1);
#endif
    ...
    lcdPrintf(fdLCD, "Tri:%d", triangleCount);
    lcdPosition(fdLCD, 0, 1);
    lcdPrintf(fdLCD, "Rect:%d", squareCount);
    lcdPosition(fdLCD, 8, 1);
    lcdPrintf(fdLCD, "Cir:%d", circleCount);
